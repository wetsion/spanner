package site.wetsion.framework.spanner.date;

import org.apache.commons.lang3.StringUtils;
import site.wetsion.framework.spanner.base.NumberUtil;
...
public class DateTimeUtil {
    ...
    // 利雅得时区
    public static final ZoneId ZONE_ID_RIYADH = ZoneId.of("Asia/Riyadh");

    // 迪拜时区
    public static final ZoneId ZONE_ID_DUBAI = ZoneId.of("Asia/Dubai");

    // 科威特时区
    public static final ZoneId ZONE_ID_KUWAIT = ZoneId.of("Asia/Kuwait");

    // 北京时区
    public static final ZoneId ZONE_ID_SHANGHAI = ZoneId.of("Asia/Shanghai");

    // 卡塔尔时区
    public static final ZoneId ZONE_ID_QATAR = ZoneId.of("Asia/Qatar");

    /**
     * 安全地转换时间字符串为时间戳，不抛异常，结果可能为空
     * @param timeStr
     * @param pattern
     * @return 时间戳
     */
    public static Long parseTimestampSafely(String timeStr, String pattern, ZoneId zoneId) {
        try {
            return parseTimestamp(timeStr, pattern, zoneId);
        } catch (Exception e) {
            return null;
        }
    }

    public static Long parseTimestampSafely(String timeStr, String pattern) {
        return parseTimestampSafely(timeStr, pattern, ZoneOffset.systemDefault());
    }

    public static String fromMicroSoftExcelTimeSafely(String date, String pattern, ZoneId zoneId) {
        try {
            if (!NumberUtil.isNumber(date)) {
                return null;
            }
            BigDecimal bigDecimal = new BigDecimal(date);
            int days = bigDecimal.intValue();
            int mils = (int) Math.round(bigDecimal.subtract(new BigDecimal(days)).doubleValue() * 24 * 3600);
            Calendar calendar = Calendar.getInstance();
            calendar.set(1900, 0, 1);
            calendar.add(Calendar.DATE, days - 1);
            int hour = mils / 3600;
            int minute = (mils - hour * 3600) / 60;
            int second = mils - hour * 3600 - minute * 60;
            calendar.set(Calendar.HOUR_OF_DAY, hour);
            calendar.set(Calendar.MINUTE, minute);
            calendar.set(Calendar.SECOND, second);
            Date date1 = calendar.getTime();
            return fromTimestamp(date1.getTime() / 1000, pattern, zoneId);
        } catch (Exception e) {
            return null;
        }
    }

    public static String fromMicroSoftExcelTimeSafely(String date, String pattern) {
        return fromMicroSoftExcelTimeSafely(date, pattern, ZoneOffset.systemDefault());
    }
    ...
    private long getCurrentMonthBeginTimestamp() {
        // 获取当前日期，然后设置为当前月的第一天
        ...
        // 将LocalDateTime转换为ZonedDateTime，此时会考虑时区差异
        ZonedDateTime zdt = startOfDay.atZone(ZoneId.of("Asia/Shanghai"));
     ...
     * 获取当前月的第一秒时间戳
     */
    public static long getFirstSecondTimestampOfCurrentMonth() {
        return getFirstSecondTimestampOfCurrentMonth(ZoneId.of("Asia/Shanghai"));
    }

    /**
     * 获取当前月的第一秒时间戳
     */
    public static long getFirstSecondTimestampOfCurrentMonth(ZoneId zoneId) {
        // 获取当前日期，然后设置为当前月的第一天
        LocalDate firstDayOfMonth = LocalDate.now().withDayOfMonth(1);
        ...
        // 将LocalDateTime转换为ZonedDateTime，此时会考虑时区差异
        ZonedDateTime zdt = startOfDay.atZone(zoneId);
     ...
     * 获取上个月的第一秒时间戳
     */
    public static long getFirstSecondTimestampOfLastMonth() {
        return getFirstSecondTimestampOfLastMonth(ZoneId.of("Asia/Shanghai"));
    }

    /**
     * 获取上个月的第一秒时间戳
     */
    public static long getFirstSecondTimestampOfLastMonth(ZoneId zoneId) {
        // 获取当前日期
